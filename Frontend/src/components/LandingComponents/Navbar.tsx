      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between gap-6 md:gap-12 flex-wrap">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {['home','trendings', 'category',  'testimonials'].map((section) => (
              <ScrollLink
                key={section}
                to={section}
                smooth={true}
                duration={500}
                offset={-60}
                className={`rounded-full p-3 transition-colors duration-300 hover:bg-black cursor-pointer ${
                  isScrolled ? 'text-black font-medium hover:text-white ' : 'text-white hover:bg-white hover:text-black'
                }`}
                onClick={() => setIsOpen(false)}
              >
          ...
          <div className="hidden md:block flex gap-4">
            ...
            <button
              className={`${isScrolled ? 'text-black bg-black px-5 py-2 text-white rounded hover:cursor-pointer' : 'bg-white hover:cursor-pointer text-black px-5 py-2 rounded hover:opacity-90 transition'} focus:outline-none`}
